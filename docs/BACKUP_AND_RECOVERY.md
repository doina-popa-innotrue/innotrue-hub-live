...
The following cron jobs are defined in migrations and will be restored by `supabase db push`:

| Job | Schedule | Function invoked |
|-----|----------|------------------|
| `cleanup-notifications` | Daily | SQL function |
| `send-schedule-reminders` | Every 15 min | `send-schedule-reminders` edge function |
| `subscription-reminders` | Daily | `subscription-reminders` edge function |
| `registration-follow-ups` | Daily | `registration-follow-ups` edge function |
| `decision-reminders` | Daily | `decision-reminders` edge function |
| `analytics-ai-insights` | Daily | `analytics-ai-insights` edge function |
| `session-reminders` (G6) | Every 15 min | `send-session-reminders` edge function |

These jobs call edge functions via `pg_net` HTTP requests to the Supabase functions URL. After a project recreation, the jobs will target the new project URL automatically (they use `SUPABASE_URL` internally).
...
6. **Restore storage:** Upload from backup (`supabase storage cp -r ...`)
7. **Update hosting:**
   - Cloudflare Pages: update `VITE_SUPABASE_URL` and `VITE_SUPABASE_PUBLISHABLE_KEY`
   - Update `SITE_URL` edge function secret
8. **Update webhooks:** Re-register all webhook URLs with new project ref (see Section 7)
9. **Update OAuth:** Re-register redirect URIs with new project ref (see Section 7)
10. **Update Auth settings:** Configure email provider, Google provider (see Section 6)
11. **Verify:** Test all critical flows + integrations
12. **Data:** User data, enrollments, etc. are lost unless you have a database dump. PITR only works within the same project.

...
| `docs/CRON_JOBS.md` | **Cron jobs** — 9 pg_cron jobs (schedule, monitoring queries, management, recovery). Idempotent restore migration: `20260228120000_restore_all_cron_jobs.sql` |
...
## Database Schema
- 380+ tables, 24 enums (`goal_category` removed — replaced with TEXT + FK to `wheel_categories`), 483 migrations
...
- **Admin Tier Upgrade Requests (2026-04-13):** New admin page at `/admin/tier-upgrade-requests`. Status filtering, review dialog, approve/decline + admin notes. Approval updates `client_enrollments.tier` automatically. Added notification types `tier_upgrade_requested` + `tier_upgrade_approved`. Sidebar in Monetization section.
- **Supabase Audit Fixes (2026-04-13):** Fixed 2 audit warnings — `is_session_group_member` SECURITY DEFINER missing `SET search_path = ''` (allowed `auth.uid()` shadowing), `enrollment_module_staff` UPDATE policy had `WITH CHECK (true)` allowing row reassignment. Migration `20260413120000`. Commit `01a138e`.
- **B2C Email Journey Fixes (2026-04-13):** Two critical gaps in client email journey fixed: (1) `redeem-enrollment-code` now notifies the CLIENT (not just admin) using existing `program_enrolled` notification type; (2) `complete-registration` now triggers welcome email for OAuth users (who bypass `verify-signup`). Commit `5595b8a`.
- **CTA Enrollment Codes + Enrolments Source Filter (2026-04-13):** Created 4 free multi-use enrollment codes for B2C launch (CTAINNOTRUE-E/P, CTACLOUDEARLY-E/P) with tier grants. Enhanced `/admin/enrolments` with enrollment code filter dropdown, source column in table (shows code badge or enrollment_source), and source fields in CSV export. Migration `20260413130000`. Commits `72a6876`, `8b80a40`.

...
- **Schema Drift Audit (2026-03-23):** Comprehensive audit found widespread code-to-DB mismatches. Full report: `docs/SCHEMA_DRIFT_AUDIT.md`. Key findings:
  - 🔴 **`profiles.email` MISSING** — 16+ files reference it, column doesn't exist. `profiles.username` stores auth email (via `handle_new_user`), some newer code uses it correctly. `profiles.real_email` is for placeholder users ONLY (admin tracking). Decision pending: add `email` column vs change code to `username`.
  - 🔴 **`profiles.is_disabled` MISSING** — 5 files reference it, column doesn't exist. Disabled state lives in `auth.users.banned_until` (set by `delete-user` edge function, checked by `get-user-email`). All `is_disabled` checks evaluate to `undefined` → **disabled users receive all emails**. Decision pending: add column (denormalized from auth ban) vs rewrite checks.
  - 🟠 **`profiles.full_name` → should be `name`** — 8 references across 5 frontend files + 1 edge function + 1 DB function. Names show as fallbacks ("Instructor", "Student", null).
  - 🟠 **DB trigger bugs** — `notify_session_participant_added()` references `ms.coach_id` (doesn't exist on `module_sessions`), `staff_has_client_relationship()` missing `enrollment_module_staff` check, `notify_program_assignment()` uses `p.email`/`p.is_disabled` (don't exist).
  ...
  - 🟡 **Phantom tables** — 6 tables referenced in code never existed: `client_sessions`, `user_subscriptions`, `organization_subscriptions`, `wheel_of_life_scores`, `user_organization_sharing_consent`, `instructor_assignments`. Affects: `calendar-feed` (10+ issues, mostly non-functional), `credit-maintenance` (no cron trigger), `generate-reflection-prompt`, `accept-org-invite`, `useReadinessDashboard`.
  - 🔵 **`credit-maintenance` has no cron trigger** — function exists but nothing invokes it. Credits may not be expiring as intended.
- **Next steps:** **Schema drift fixes (Sprint 1)** → 2B.5 Certification → 2B.10 Enrollment Duration → 2B.13 Credit Expiry Policy Migration → 2B.11 Feature Loss Communication → Phase 5 remaining (Wheel pipeline, bulk import) → Phase 3 AI

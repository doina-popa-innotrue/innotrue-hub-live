...
export function useConsumableAddOns() {
  ...
  const hasCredits = useCallback(
    async (addOnKey: string, requiredAmount: number = 1): Promise<boolean> => {
   ...
   */
  const consumeWithFeedback = useCallback(
    async (
      addOnKey: string,
      ...
      if (!result.success) {
        if (result.error === "Insufficient credits") {
          toast.error(
            `You don't have enough credits for this action. Remaining: ${result.remaining || 0}`,
          );
        } else {

...
export function useScenarioProgress(templateId: string | undefined, assignmentId: string | undefined) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const answeredParagraphIds = new Set(
        responses?.filter(r => r.response_text && r.response_text.trim().length > 0)

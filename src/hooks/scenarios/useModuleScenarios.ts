...
export function useModuleScenarioMutations() {
  ...
  const removeMutation = useMutation({
    mutationFn: async ({ id, moduleId }: { id: string; moduleId: string }) => {
      const { error } = await supabase.from("module_scenarios").delete().eq("id", id);
  ...
  const updateMutation = useMutation({
    mutationFn: async ({
      ...
      const { error } = await supabase.from("module_scenarios").update(updates).eq("id", id);
  ...
  const reorderMutation = useMutation({
    mutationFn: async ({ moduleId, orderedIds }: { moduleId: string; orderedIds: string[] }) => {
      ...
      const updates = orderedIds.map((id, index) =>
        supabase.from("module_scenarios").update({ order_index: index }).eq("id", id),
      );

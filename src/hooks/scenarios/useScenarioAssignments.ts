...
export function useScenarioAssignments(filters?: {
  userId?: string;
  templateId?: string;
  status?: string;
}) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const assignedByIds = [
        ...new Set(data?.filter((a) => a.assigned_by).map((a) => a.assigned_by!) || []),
      ];
      const evaluatedByIds = [
        ...new Set(data?.filter((a) => a.evaluated_by).map((a) => a.evaluated_by!) || []),
      ];
      ...
      return (data || []).map((assignment) => ({
        ...
        assigned_by_profile: assignment.assigned_by
          ...
          : null,
        evaluated_by_profile: assignment.evaluated_by
          ? profileMap.get(assignment.evaluated_by) || null
          : null,
...
export function useScenarioAssignmentMutations() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("scenario_assignments").delete().eq("id", id);

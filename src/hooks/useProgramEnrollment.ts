...
export function useProgramEnrollment() {
  ...
  const getTierCreditCost = useCallback((programId: string, tierName: string): number | null => {
    const key = `${programId}:${tierName}`;
    return tierPricing.get(key)?.credit_cost ?? null;
  }, [tierPricing]);

  // Check if user can afford to enroll in a tier
  const canAffordTier = useCallback((programId: string, tierName: string): { 
    canAfford: boolean; 
      ...
      const { data: enrollment, error: enrollError } = await supabase
        ...
        .insert({
          ...
          discount_percent: discountResult 
            ? Math.round(((originalCreditCost! - discountResult.discountedCost) / originalCreditCost!) * 100)
            : null,

...
export function useExploreModuleCompletions(userId: string | undefined) {
  ...
  const checkProgramCompletions = useCallback(
    async (programId: string): Promise<ProgramCompletionInfo> => {
      if (!userId) {
      ...
      try {
        ...
        const userCompletedProgress = (otherProgramProgress || []).filter(
          (p: any) =>
        ...
        const canonicalCodeToCompletion = new Map<
          string,
          { programName: string; completedAt: string | null }
        >();
        ...
        modules.forEach((module) => {
          ...
          // Check by TalentLMS link in module links field
            ...
            try {
              const links =
                typeof module.links === "string" ? JSON.parse(module.links) : module.links;

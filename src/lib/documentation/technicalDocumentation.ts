...
const createParagraph = (text: string, options?: { bold?: boolean; italic?: boolean; bullet?: boolean }) => {
  return new Paragraph({
    children: [
      new TextRun({
        text,
...
export const generateTechnicalDocumentation = (): Document => {
  return new Document({
    ...
    description: 'Full stack technical documentation including database schema, APIs, and architecture',
        ...
        children: [
          ...
          createTable(
            ...
            [
              ...
              ['user_roles', 'Role assignments', 'user_id, role (admin/instructor/coach/client/org_admin)'],
              ['client_profiles', 'Additional client data', 'user_id, status, status_marker, notes, tags'],
              ['client_coaches', 'Coach-client relationships', 'client_id, coach_id'],
              ['billing_info', 'Billing details (protected)', 'user_id, company, vat, address fields'],
              ['ai_preferences', 'User AI consent and preferences', 'user_id, ai_insights_enabled, consent_given_at'],
            ]
          ),
          createParagraph(''),
          ...
          createTable(
            ['Table', 'Description', 'Key Columns'],
            [
              ['programs', 'Learning programs', 'id, name, slug, description, category, min_plan_tier'],
              ['program_modules', 'Program modules', 'id, program_id, title, type, order_index, content'],
              ['client_enrollments', 'User enrollments', 'id, client_user_id, program_id, status, tier, credit_cost'],
              ['module_progress', 'Completion tracking', 'id, enrollment_id, module_id, status, completed_at'],
              ['module_sessions', 'Session scheduling', 'id, module_progress_id, session_date, status, instructor_id, coach_id'],
              ['program_versions', 'Version history', 'id, program_id, version_number, module_snapshot'],
              ['program_plans', 'Program tiers', 'id, program_id, name, credit_allowance'],
              ['enrollment_module_staff', 'Staff assignments to modules', 'enrollment_id, module_id, staff_id'],
            ]
          ),
          createParagraph(''),
          ...
          createTable(
            ['Table', 'Description', 'Key Columns'],
            [
              ['features', 'Feature definitions', 'id, key, name, is_consumable, is_system, is_active'],
              ['plans', 'Subscription plans', 'id, key, name, tier_level, credit_allowance'],
              ['plan_features', 'Plan-feature mapping', 'plan_id, feature_id, enabled, limit_value'],
              ['tracks', 'Learning tracks', 'id, key, name, display_name'],
              ['track_features', 'Track-feature mapping', 'track_id, feature_id, is_enabled, limit_value'],
              ...
              ['user_add_ons', 'User add-on assignments', 'user_id, add_on_id, remaining_quantity, expires_at'],
            ]
          ),
          createParagraph(''),
          ...
          createTable(
            ['Table', 'Description', 'Key Columns'],
            [
              ['credit_batches', 'Credit allocations', 'owner_id, source_type, original_amount, remaining_amount, expires_at'],
              ...
              ['credit_usage_periods', 'Usage tracking', 'owner_id, period_start, period_end, credits_used'],
            ]
          ),
          createParagraph(''),
          ...
          createTable(
            ['Table', 'Description', 'Key Columns'],
            [
              ['wheel_snapshots', 'Wheel of Life assessments', 'id, user_id, snapshot_date, ratings (by category)'],
              ['wheel_categories', 'Configurable wheel domains', 'id, key, name, color, icon, order_index, is_active'],
              ['capability_assessments', 'Assessment definitions', 'id, name, slug, rating_scale, family_id'],
              ['capability_domains', 'Assessment domains', 'id, assessment_id, name, order_index'],
              ['capability_domain_questions', 'Domain questions', 'id, domain_id, question_text, input_type'],
              ['capability_snapshots', 'Assessment results', 'id, assessment_id, user_id, status, completed_at, shared_with_coach, shared_with_instructor'],
              ['capability_snapshot_ratings', 'Question ratings', 'id, snapshot_id, question_id, rating'],
              ['capability_domain_notes', 'Domain-level notes', 'id, snapshot_id, domain_id, content'],
              ['capability_question_notes', 'Question-level notes', 'id, snapshot_id, question_id, content'],
          ...
          createTable(
            ['Table', 'Description', 'Key Columns'],
            [
              ['scenario_templates', 'Scenario definitions', 'id, title, description, capability_assessment_id, is_protected, is_locked'],
              ['scenario_sections', 'Template sections/pages', 'id, template_id, title, instructions, order_index'],
              ['section_paragraphs', 'Paragraph content', 'id, section_id, content, order_index, requires_response'],
              ['paragraph_question_links', 'Links paragraphs to capability questions', 'id, paragraph_id, question_id, weight'],
              ['module_scenarios', 'Links scenarios to modules', 'id, module_id, template_id, is_required_for_certification, order_index'],
              ['scenario_assignments', 'Client assignments', 'id, template_id, user_id, enrollment_id, module_id, status'],
              ['paragraph_responses', 'Client responses', 'id, assignment_id, paragraph_id, response_text'],
              ['paragraph_evaluations', 'Evaluator feedback', 'id, assignment_id, paragraph_id, evaluator_id, feedback'],
              ['paragraph_question_scores', 'Question scores', 'id, assignment_id, paragraph_id, question_id, score, evaluator_id'],
            ]
          ),
          createParagraph(''),
          ...
          createTable(
            ['Table', 'Description', 'Key Columns'],
            [
              ['goals', 'User goals', 'id, user_id, title, category, status, progress, target_date'],
              ['goal_milestones', 'Goal milestones', 'id, goal_id, title, target_date, completed_at'],
              ['tasks', 'Action items', 'id, user_id, goal_id, title, status, due_date, priority'],
              ['decisions', 'Decision records', 'id, user_id, title, context, status, chosen_option'],
          ...
          createTable(
            ['Table', 'Description', 'Key Columns'],
            [
              ['groups', 'Learning groups', 'id, name, description, program_id, is_active, access_type'],
              ['group_members', 'Group membership', 'id, group_id, user_id, role (leader/member)'],
              ['group_sessions', 'Scheduled sessions', 'id, group_id, title, scheduled_at, meeting_link'],
          ...
          createTable(
            ['Table', 'Description', 'Key Columns'],
            [
              ['email_templates', 'Email template definitions', 'id, name, subject, body_html, body_text'],
              ['email_queue', 'Pending/sent emails', 'id, to_email, template_id, status, sent_at, error_message'],
          ...
          createTable(
            ['Table', 'Description', 'Key Columns'],
            [
              ['calcom_event_type_mappings', 'Cal.com event type configs', 'id, calcom_event_type_id, session_target, default_program_id, default_module_id'],
              ['calcom_webhook_logs', 'Webhook processing logs', 'id, event_type, payload, processed, error_message'],
              ['instructor_calcom_event_types', 'Instructor-specific Cal.com mappings', 'id, instructor_id, calcom_event_type_id, scheduling_url'],
            ]
          ),
          new Paragraph({ children: [new PageBreak()] }),
          ...
          createTable(
            ...
            [
              ['trigger_notify_session_scheduled', 'module_sessions', 'Sends email when session status changes to scheduled with a date'],
              ['trigger_notify_assignment_submitted', 'module_progress', 'Notifies staff when assignment is submitted'],
              ['update_*_updated_at', 'Various', 'Auto-updates updated_at timestamp on row changes'],
          ...
          createParagraph('Triggers when module_sessions status changes to "scheduled" with a valid session_date:'),
          createParagraph('Resolves client information (name, email) from enrollment', { bullet: true }),
          createParagraph('Resolves staff name from instructor_id OR coach_id', { bullet: true }),
          createParagraph('Sends notification via send-notification-email edge function', { bullet: true }),
          createParagraph('Includes scheduledDate, staff name, meeting link, and program context', { bullet: true }),
          ...
          createTable(
            ...
            [
              ...
              ['staff_has_client_relationship(staff_id, client_id)', 'Check coach/instructor assignment'],
          ...
          createCodeBlock('  userRole: "admin" | "coach" | "client" | "instructor" | "org_admin" | null;'),
          ...
          createParagraph('Roles are stored in user_roles table (separate from profiles for security).'),
          createParagraph('Frontend: ProtectedRoute component wraps authenticated pages', { bullet: true }),
          createParagraph('Backend: RLS policies check auth.uid() against user_id columns', { bullet: true }),
          createParagraph('Admin check: has_role(auth.uid(), \'admin\') function verifies admin role', { bullet: true }),
          createParagraph('Staff check: staff_has_client_relationship() for coach/instructor access', { bullet: true }),
          ...
          createParagraph('Serverless functions deployed to Supabase Edge Functions (Deno runtime). 60+ functions available.'),
          
          createHeading('Email & Notifications', HeadingLevel.HEADING_2),
          createTable(
            ['Function', 'Method', 'Purpose'],
            [
              ['send-notification-email', 'POST', 'Send system notifications (sessions, reminders, etc.)'],
          ...
          createParagraph('UI Components (src/components/ui/): shadcn/ui components with custom variants', { bullet: true }),
          createParagraph('Feature Components: Organized by feature domain (admin, modules, goals, etc.)', { bullet: true }),
          createParagraph('Layout Components: DashboardLayout, OrgAdminLayout, AppSidebar', { bullet: true }),
          createParagraph('Page Components (src/pages/): Route-level components organized by role', { bullet: true }),
          ...
          createParagraph('index.css: CSS custom properties for colors, spacing, etc.', { bullet: true }),
          createParagraph('tailwind.config.ts: Extended theme configuration', { bullet: true }),
          createParagraph('All colors use HSL format with semantic naming (primary, secondary, muted, etc.)', { bullet: true }),
          createParagraph('NEVER use hardcoded colors (bg-white, text-black) - always use design tokens', { bullet: true }),
          new Paragraph({ children: [new PageBreak()] }),
          ...
          createParagraph('localStorage for persistent preferences (e.g., cookie consent, completed tours)'),
          new Paragraph({ children: [new PageBreak()] }),
          ...
          createParagraph('Form components: Input, Select, Checkbox, Radio, Switch, Textarea', { bullet: true }),
          createParagraph('Feedback: Toast, Alert, Badge, Progress, Skeleton', { bullet: true }),
          createParagraph('Navigation: Breadcrumb, Sidebar, Navigation Menu, Dropdown Menu', { bullet: true }),
          ...
          createParagraph('InstructorCalcomEventTypes: Instructor Cal.com mapping UI', { bullet: true }),
          ...
          createParagraph('Features are defined in the features table with keys used for access checks.'),
          createParagraph('Features can be assigned via: plans, tracks, add-ons, or program plans.'),
          ...
          createParagraph('Database triggers using pg_net to call edge functions', { bullet: true }),
          ...
          createCodeBlock('    url := get_secret(\'SUPABASE_URL\') || \'/functions/v1/send-notification-email\','),
          ...
          createParagraph('User Mapping: talentlms_users table links platform users to LMS accounts'),
          ...
          createParagraph('All tables have Row Level Security (RLS) policies enabled', { bullet: true }),
          createParagraph('Roles stored in separate user_roles table (not profiles) to prevent escalation', { bullet: true }),
          createParagraph('Staff access restricted to assigned clients via staff_has_client_relationship()', { bullet: true }),
          ...
          createParagraph('Privacy policy with international transfer disclosures', { bullet: true }),
          ...
          createParagraph('NOTE: Authentication context management UI is planned for future development. Current configuration is database-driven.'),
          createParagraph(''),
          ...
          createTable(
            ['Function', 'Method', 'Purpose'],
            [
              ['signup-user', 'POST', 'Create new user with context metadata (program, track, org, UTM params)'],
              ['verify-signup', 'POST', 'Validate email verification token and create profile/roles'],
              ['send-auth-email', 'POST', 'Send templated auth emails (signup confirm, password reset, etc.)'],
            ]
          ),
          ...
          createTable(
            ...
            [
              ['auth_contexts', 'Store landing page configurations', 'slug, headline, features (JSONB), program_id, track_id, organization_id'],
              ['signup_contexts', 'Track user signup attribution', 'user_id, context_slug, utm_source/medium/campaign, program_enrolled, organization_joined'],
            ]
          ),
          ...
          createCodeBlock('const contextSlug = searchParams.get("ref") || searchParams.get("context");'),
          ...
          createParagraph('RLS on auth_contexts: Public read for active contexts, admin-only write', { bullet: true }),
          createParagraph('RLS on signup_contexts: Users can only read/insert their own records', { bullet: true }),
          ...
          createParagraph('The platform uses Supabase Storage (S3-compatible) for all file uploads. Files are never stored in the database — only URLs/references are saved.'),
          createParagraph(''),
          ...
          createTable(
            ...
            [
              ...
              ['program-logos', 'Program branding and badge images', 'Yes', 'Public read, admin/instructor manage'],
              ['email-assets', 'Email template images (logo, graphics)', 'Yes', 'Public read, admin manage'],
              ['resource-library', 'Admin-managed downloadable resources', 'No', 'Authenticated read, admin manage'],
              ['goal-resources', 'Goal and milestone attachments', 'No', 'Owner read/write, staff view'],
              ['module-client-content', 'Client module submissions', 'No', 'Owner + assigned staff access'],
              ['module-assignment-attachments', 'Assignment file uploads', 'No', 'Owner + assigned staff access'],
              ['module-reflection-resources', 'Reflection attachments', 'No', 'Owner + assigned staff access'],
              ...
              ['client-badges', 'Issued badge images for clients', 'No', 'Owner read, admin/instructor issue'],
              ['group-notes', 'Group session notes and files', 'No', 'Group members + leaders access'],
              ['psychometric-assessments', 'Assessment-related files', 'No', 'Owner + staff access'],
              ['external-course-certificates', 'External course completion certificates', 'No', 'Owner read/write, admin view'],
            ]
          ),
          createParagraph(''),
          ...
          createCodeBlock('https://{project-id}.supabase.co/storage/v1/object/public/{bucket}/{path}'),
          createCodeBlock(''),
          createCodeBlock('// Private bucket - requires signed URL'),
          createCodeBlock('const { data } = await supabase.storage.from("bucket").createSignedUrl(path, 3600);'),
          createParagraph(''),
          ...
          createParagraph('Public buckets: Allow anyone to read via direct URL, but only authorized users can upload/delete', { bullet: true }),
          createParagraph('Private buckets: Require authentication for all operations, use signed URLs for temporary access', { bullet: true }),
          createParagraph('RLS policies on storage.objects control access based on bucket_id and user context', { bullet: true }),
          createParagraph('File paths typically include user ID or entity ID for scoping (e.g., avatars/{user_id}/photo.jpg)', { bullet: true }),
          createParagraph('Maximum file sizes are enforced per bucket via RLS or application logic', { bullet: true }),

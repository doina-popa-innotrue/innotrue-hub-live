...
describe('buildCalcomBookingUrl', () => {
  it('throws if schedulingUrl is empty', () => {
    expect(() => buildCalcomBookingUrl({ schedulingUrl: '' })).toThrow('Scheduling URL is required');
  });
...
describe('extractEventTypeSlug', () => {
  ...
  it('extracts slug from longer paths', () => {
    expect(extractEventTypeSlug('https://cal.com/team/org/coaching-session')).toBe('coaching-session');
  });
...
describe('buildCalcomRescheduleUrl', () => {
  ...
  it('adds successUrl for redirect', () => {
    const url = buildCalcomRescheduleUrl('booking-123', 'https://cal.com', 'https://app.example.com/done');
    const parsed = new URL(url);

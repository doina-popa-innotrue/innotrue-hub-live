...
export function ProgramPlanConfig({ 
  ...
  const [requiresSeparatePurchase, setRequiresSeparatePurchase] = useState(currentRequiresSeparatePurchase);
  ...
  }, [currentPlanId, currentMinTier, currentRequiresSeparatePurchase, currentAllowRepeatEnrollment]);
    ...
    setTierPlanMappings(prev => 
      prev.map(m => 
        m.tier_name === tierName 
          ? { ...m, program_plan_id: planId === 'none' ? null : planId }
          : m
      )
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className={requiresSeparatePurchase ? 'border-amber-500 bg-amber-50/50 dark:bg-amber-950/20' : ''}>
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground">
                Users cannot access this program through any subscription plan. They must be manually enrolled after payment.
              </p>
            </div>
            <Switch
          ...
          {requiresSeparatePurchase && (
            <Alert className="bg-amber-100 border-amber-300 dark:bg-amber-900/30 dark:border-amber-700">
              ...
              <AlertDescription className="text-amber-800 dark:text-amber-200">
                This program will be visible in the catalogue but locked for all users. Access is granted only through manual enrollment by an admin.
              </AlertDescription>
            </Alert>
          )}
        </CardContent>
      ...
      <Card className={allowRepeatEnrollment ? 'border-primary/50 bg-primary/5' : ''}>
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Allow clients to enroll in this program multiple times (e.g., for mock review boards, practice programs)
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground">
                Clients can enroll again after completing or while still active. Each enrollment has separate progress and assignments.
              </p>
            </div>
            <Switch
          ...
          {allowRepeatEnrollment && (
            <Alert className="bg-primary/10 border-primary/20">
              ...
              <AlertDescription className="text-foreground">
                Clients will see an "Enroll Again" option on the Explore page after their first enrollment. Each attempt is tracked with an enrollment number.
              </AlertDescription>
            </Alert>
          )}
        </CardContent>
        ...
        <CardContent className="space-y-4">
          ...
          {programTiers.length === 0 && (
            <p className="text-sm text-muted-foreground bg-muted p-3 rounded-md">
              No tiers configured for this program. Add tiers in the Settings tab to assign different program plans per tier.
            </p>
          )}

          {programPlans.length === 0 && (
            <p className="text-sm text-muted-foreground bg-muted p-3 rounded-md">
              No program plans configured yet. Create program plans in Admin → Plans & Monetization → Program Plans.
            </p>
          )}
        </CardContent>
      </Card>

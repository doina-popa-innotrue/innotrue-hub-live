...
export function InstructorCalcomEventTypes() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      const { error } = await supabase.from("instructor_calcom_event_types").insert({
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("instructor_calcom_event_types").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["instructor-calcom-event-types"] });
  ...
  const groupedByInstructor = mappings.reduce(
    (acc, mapping) => {
    ...
    {} as Record<
      string,
      { instructor?: InstructorEventType["instructor"]; mappings: InstructorEventType[] }
    >,
  );
      ...
      <CardHeader className="flex flex-col space-y-4 pb-4">
        <div className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              Map each instructor's Cal.com child event type ID per module type for personalized
              booking
            </CardDescription>
          </div>
          <Dialog
          ...
          >
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>
                  {editingMapping
                    ? "Edit Instructor Event Type Mapping"
                    : "Add Instructor Event Type Mapping"}
                </DialogTitle>
              </DialogHeader>
              <form onSubmit={handleSubmit} className="space-y-4">
                {!editingMapping ? (
                  ...
                  <div className="rounded-lg border p-3 bg-muted/30">
                    <p className="text-sm text-muted-foreground">
                      Editing mapping for <strong>{editingMapping.instructor?.name}</strong> on{" "}
                      <strong>{editingMapping.module_type}</strong>
                    </p>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, child_event_type_id: e.target.value })
                    }
                    placeholder="e.g., 789012"
                  />
                  <p className="text-xs text-muted-foreground">
                    Use the <strong>instructor's child event type ID</strong> (not the parent team
                    ID). Find this in Cal.com → Team → Managed Event → Click the specific
                    instructor's version to get their unique ID.
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    <strong>For managed events:</strong> Copy the full booking URL from Cal.com. The
                    system will use this directly instead of making API calls. Leave empty if you
                    want the system to resolve the URL automatically via the Cal.com API.
                  </p>
                </div>
        ...
        <div className="rounded-lg border p-3 bg-muted/30 text-sm">
          <p className="text-muted-foreground">
            <strong className="text-foreground">Purpose:</strong> These mappings generate{" "}
            <strong>personalized booking URLs</strong> for clients. When a client needs to book a
            session, the system uses this table to find the instructor's specific Cal.com Child
            Event Type ID for that module type, ensuring correct session duration and calendar
            routing.
          </p>
        </div>
      </CardHeader>
        ...
        {mappings.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-sm mt-1">
              Add mappings to enable personalized booking URLs per instructor
            </p>
          </div>
        ) : (
          <div className="space-y-6">
            {Object.entries(groupedByInstructor).map(
              ([instructorId, { instructor, mappings: instructorMappings }]) => (
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {instructorMappings.length} module type
                        {instructorMappings.length !== 1 ? "s" : ""} configured
                      </div>
                    </div>
                  </div>
                  <Table>
                              ...
                              <AlertDialog>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      Remove this event type mapping for {instructor?.name} on{" "}
                                      {mapping.module_type}?
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>

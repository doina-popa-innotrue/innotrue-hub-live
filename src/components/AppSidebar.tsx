import { Users, BookOpen, LayoutDashboard, Shield, Link2, Layers, UserCog, GraduationCap, Target, Brain, CheckSquare, BarChart3, Sparkles, DollarSign, Zap, Calendar, ClipboardList, ClipboardCheck, FileText, ExternalLink, UsersRound, ScrollText, Package, ChevronDown, Award, MessageSquare, Mail, HelpCircle, Settings, UserX, CircleDot, UserCheck, Search, GitBranch, Lightbulb, ListChecks, FolderOpen, Building2, Coins, Tag, Map, Bell, Image, Send, Lock, Megaphone } from 'lucide-react';
import { NavLink } from '@/components/NavLink';
import { useLocation, useNavigate } from 'react-router-dom';
import { useAuth } from '@/contexts/AuthContext';
import { Sidebar, SidebarContent, SidebarGroup, SidebarGroupContent, SidebarMenu, SidebarMenuButton, SidebarMenuItem, SidebarMenuSub, SidebarMenuSubItem, SidebarMenuSubButton, SidebarTrigger, useSidebar } from '@/components/ui/sidebar';
...
import { useMultipleFeatureVisibility, FeatureVisibilityResult } from '@/hooks/useFeatureVisibility';
...
export function AppSidebar() {
  ...
  const {
    loginToTalentLms,
    isLoading: talentLmsLoading
  } = useTalentLmsSSO();
  const {
    loginToLucid,
    isLoading: lucidLoading
  } = useLucidSSO();
  const {
    openGoogleDrive,
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data
      } = await supabase.from('circle_users').select('*').eq('user_id', user.id).maybeSingle();
      return data;
    },
    enabled: !!user?.id && (userRoles.includes('client') || userRoles.includes('instructor') || userRoles.includes('coach'))
  });
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data
      } = await supabase.from('talentlms_users').select('*').eq('user_id', user.id).maybeSingle();
      return data;
    },
    enabled: !!user?.id && (userRoles.includes('client') || userRoles.includes('instructor') || userRoles.includes('coach'))
  });
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data
      } = await supabase.from('lucid_users').select('*').eq('user_id', user.id).maybeSingle();
      return data;
    },
    enabled: !!user?.id && (userRoles.includes('client') || userRoles.includes('instructor') || userRoles.includes('coach'))
  });
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data
      } = await supabase.from('miro_users').select('*').eq('user_id', user.id).maybeSingle();
      return data;
    },
    enabled: !!user?.id && (userRoles.includes('client') || userRoles.includes('instructor') || userRoles.includes('coach'))
  });
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data
      } = await supabase.from('mural_users').select('*').eq('user_id', user.id).maybeSingle();
      return data;
    },
    enabled: !!user?.id && (userRoles.includes('client') || userRoles.includes('instructor') || userRoles.includes('coach'))
  });
  ...
  const handleLockedItemClick = (item: NavItemWithFeature, visibility: FeatureVisibilityResult, e: React.MouseEvent) => {
    e.preventDefault();
                ...
                return (
                  <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton asChild>
                      <NavLink to={item.url} end={item.url === '/dashboard' || item.url === '/admin' || item.url === '/teaching'} className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground rounded-md" activeClassName="bg-primary text-primary-foreground font-medium rounded-md" data-tour={(item as any).tourId}>
                        <item.icon className="mr-2 h-4 w-4" />
                        {open && <span>{item.title}</span>}
                      </NavLink>
                    </SidebarMenuButton>
                    ...
                    <SidebarMenuItem>
                      <CollapsibleTrigger asChild>
                        <SidebarMenuButton className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground">
                          ...
                          {open && <ChevronDown className="h-4 w-4 transition-transform group-data-[state=open]/teaching-platforms:rotate-180" />}
                        </SidebarMenuButton>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <SidebarMenuSub>
                          <SidebarMenuSubItem>
                            {talentLmsUser ? (
                              ...
                              >
                                ...
                                {open && <span className="flex-1 text-left leading-tight">InnoTrue Academy</span>}
                                {open && <ExternalLink className="h-3 w-3 opacity-50 shrink-0 mt-0.5" />}
                              </button>
                            ) : (
                          ...
                          <SidebarMenuSubItem>
                            {lucidUser ? (
                              ...
                              >
                                ...
                                {open && <span className="flex-1 text-left leading-tight">Lucid</span>}
                                {open && <ExternalLink className="h-3 w-3 opacity-50 shrink-0 mt-0.5" />}
                              </button>
                            ) : (
                              <span className="flex w-full items-start gap-2 rounded-md px-2 py-2 text-sm text-sidebar-foreground opacity-50 cursor-not-allowed">
                          ...
                          <SidebarMenuSubItem>
                            {driveUser ? (
                              ...
                              >
                                <Link2 className="h-4 w-4 shrink-0 mt-0.5" />
                                {open && <span className="flex-1 text-left leading-tight">Google Drive</span>}
                                {open && <ExternalLink className="h-3 w-3 opacity-50 shrink-0 mt-0.5" />}
                              </button>
                            ) : (
                              <span className="flex w-full items-start gap-2 rounded-md px-2 py-2 text-sm text-sidebar-foreground opacity-50 cursor-not-allowed">
                          ...
                          <SidebarMenuSubItem>
                            {miroUser ? (
                              ...
                              >
                                <Link2 className="h-4 w-4 shrink-0 mt-0.5" />
                                {open && <span className="flex-1 text-left leading-tight">Miro</span>}
                                {open && <ExternalLink className="h-3 w-3 opacity-50 shrink-0 mt-0.5" />}
                              </a>
                            ) : (
                              <span className="flex w-full items-start gap-2 rounded-md px-2 py-2 text-sm text-sidebar-foreground opacity-50 cursor-not-allowed">
                          ...
                          <SidebarMenuSubItem>
                            {muralUser ? (
                              ...
                              >
                                <Link2 className="h-4 w-4 shrink-0 mt-0.5" />
                                {open && <span className="flex-1 text-left leading-tight">Mural</span>}
                                {open && <ExternalLink className="h-3 w-3 opacity-50 shrink-0 mt-0.5" />}
                              </a>
                            ) : (
                              <span className="flex w-full items-start gap-2 rounded-md px-2 py-2 text-sm text-sidebar-foreground opacity-50 cursor-not-allowed">
                    ...
                    <SidebarMenuItem>
                      <CollapsibleTrigger asChild>
                        <SidebarMenuButton className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground">
                          ...
                          {open && <ChevronDown className="h-4 w-4 transition-transform group-data-[state=open]/client-planning:rotate-180" />}
                        </SidebarMenuButton>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <SidebarMenuSub>
                            ...
                            return (
                              <SidebarMenuSubItem key={item.title}>
                                <SidebarMenuSubButton asChild>
                                  <NavLink to={item.url} className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground" activeClassName="bg-secondary text-secondary-foreground font-medium" data-tour={item.tourId}>
                                    <item.icon className="mr-2 h-4 w-4" />
                                    {open && <span>{item.title}</span>}
                                  </NavLink>
                    ...
                    <SidebarMenuItem>
                      <CollapsibleTrigger asChild>
                        <SidebarMenuButton className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground">
                          ...
                          {open && <ChevronDown className="h-4 w-4 transition-transform group-data-[state=open]/client-learning:rotate-180" />}
                        </SidebarMenuButton>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <SidebarMenuSub>
                          {clientLearningItems.map(item => <SidebarMenuSubItem key={item.title}>
                              <SidebarMenuSubButton asChild>
                                <NavLink to={item.url} className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground" activeClassName="bg-secondary text-secondary-foreground font-medium" data-tour={item.tourId}>
                                  <item.icon className="mr-2 h-4 w-4" />
                                  {open && <span>{item.title}</span>}
                    ...
                    <SidebarMenuItem>
                      <CollapsibleTrigger asChild>
                        <SidebarMenuButton className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground">
                          ...
                          {open && <ChevronDown className="h-4 w-4 transition-transform group-data-[state=open]/client-assessments:rotate-180" />}
                        </SidebarMenuButton>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <SidebarMenuSub>
                          {clientAssessmentItems.map(item => (
                            <SidebarMenuSubItem key={item.title}>
                              <SidebarMenuSubButton asChild>
                                <NavLink to={item.url} end={item.end} className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground" activeClassName="bg-secondary text-secondary-foreground font-medium" data-tour={item.tourId}>
                                  <item.icon className="mr-2 h-4 w-4" />
                                  {open && <span>{item.title}</span>}
                                </NavLink>
                              </SidebarMenuSubButton>
                ...
                <SidebarMenu>
                  <Collapsible className="group/collapsible">
                    <SidebarMenuItem>
                      ...
                      <CollapsibleContent>
                        <SidebarMenuSub>
                          {adminProgramItems.map(item => <SidebarMenuSubItem key={item.title}>
                              <SidebarMenuSubButton asChild>
                                <NavLink to={item.url} className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground" activeClassName="bg-secondary text-secondary-foreground font-medium" data-tour={item.tourId}>
                                  <item.icon className="mr-2 h-4 w-4" />
                                  {open && <span>{item.title}</span>}
                    ...
                    <SidebarMenuItem>
                      <CollapsibleTrigger asChild>
                        <SidebarMenuButton className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground">
                          ...
                          {open && <ChevronDown className="h-4 w-4 transition-transform group-data-[state=open]/admin-resources:rotate-180" />}
                        </SidebarMenuButton>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <SidebarMenuSub>
                          {adminResourceItems.map(item => <SidebarMenuSubItem key={item.title}>
                              <SidebarMenuSubButton asChild>
                                <NavLink to={item.url} className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground" activeClassName="bg-secondary text-secondary-foreground font-medium" data-tour={item.tourId}>
                                  <item.icon className="mr-2 h-4 w-4" />
                                  {open && <span>{item.title}</span>}
                    ...
                    <SidebarMenuItem>
                      <CollapsibleTrigger asChild>
                        <SidebarMenuButton className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground">
                          ...
                          {open && <ChevronDown className="h-4 w-4 transition-transform group-data-[state=open]/collapsible2:rotate-180" />}
                        </SidebarMenuButton>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <SidebarMenuSub>
                          {adminPeopleItems.map(item => <SidebarMenuSubItem key={item.title}>
                              <SidebarMenuSubButton asChild>
                                <NavLink to={item.url} className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground" activeClassName="bg-secondary text-secondary-foreground font-medium" data-tour={item.tourId}>
                                  <item.icon className="mr-2 h-4 w-4" />
                                  {open && <span>{item.title}</span>}
                    ...
                    <SidebarMenuItem>
                      <CollapsibleTrigger asChild>
                        <SidebarMenuButton className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground">
                          ...
                          {open && <ChevronDown className="h-4 w-4 transition-transform group-data-[state=open]/collapsible3:rotate-180" />}
                        </SidebarMenuButton>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <SidebarMenuSub>
                          {adminMonetizationItems.map(item => <SidebarMenuSubItem key={item.title}>
                              <SidebarMenuSubButton asChild>
                                <NavLink to={item.url} className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground" activeClassName="bg-secondary text-secondary-foreground font-medium" data-tour={item.tourId}>
                                  <item.icon className="mr-2 h-4 w-4" />
                                  {open && <span>{item.title}</span>}
                    ...
                    <SidebarMenuItem>
                      <CollapsibleTrigger asChild>
                        <SidebarMenuButton className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground">
                          <FileText className="mr-2 h-4 w-4" />
                          {open && <span className="flex-1">Assessments</span>}
                          {open && <ChevronDown className="h-4 w-4 transition-transform group-data-[state=open]/collapsible4:rotate-180" />}
                        </SidebarMenuButton>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <SidebarMenuSub>
                          {adminAssessmentItems.map(item => <SidebarMenuSubItem key={item.title}>
                              <SidebarMenuSubButton asChild>
                                <NavLink to={item.url} className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground" activeClassName="bg-secondary text-secondary-foreground font-medium" data-tour={item.tourId}>
                                  <item.icon className="mr-2 h-4 w-4" />
                                  {open && <span>{item.title}</span>}
                    ...
                    <SidebarMenuItem>
                      <CollapsibleTrigger asChild>
                        <SidebarMenuButton className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground">
                          ...
                          {open && <ChevronDown className="h-4 w-4 transition-transform group-data-[state=open]/collapsible5:rotate-180" />}
                        </SidebarMenuButton>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <SidebarMenuSub>
                          {adminIntegrationItems.map(item => <SidebarMenuSubItem key={item.title}>
                              <SidebarMenuSubButton asChild>
                                <NavLink to={item.url} className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground" activeClassName="bg-secondary text-secondary-foreground font-medium">
                                  <item.icon className="mr-2 h-4 w-4" />
                                  {open && <span>{item.title}</span>}
                                </NavLink>
                              </SidebarMenuSubButton>
                    ...
                    <SidebarMenuItem>
                      <CollapsibleTrigger asChild>
                        <SidebarMenuButton className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground">
                          ...
                          {open && <ChevronDown className="h-4 w-4 transition-transform group-data-[state=open]/collapsible6:rotate-180" />}
                        </SidebarMenuButton>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <SidebarMenuSub>
                          {adminCommunicationsItems.map(item => <SidebarMenuSubItem key={item.title}>
                              <SidebarMenuSubButton asChild>
                                <NavLink to={item.url} className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground" activeClassName="bg-secondary text-secondary-foreground font-medium" data-tour={item.tourId}>
                                  <item.icon className="mr-2 h-4 w-4" />
                                  {open && <span>{item.title}</span>}
                    ...
                    <SidebarMenuItem>
                      <CollapsibleTrigger asChild>
                        <SidebarMenuButton className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground !h-auto py-2 items-start">
                          ...
                          {open && <ChevronDown className="h-4 w-4 shrink-0 mt-0.5 transition-transform group-data-[state=open]/client-external:rotate-180" />}
                        </SidebarMenuButton>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <SidebarMenuSub>
                            ...
                            <SidebarMenuSubItem>
                              {circleUser ? (
                                ...
                                >
                                  ...
                                  {open && <span className="flex-1 text-left leading-tight">InnoTrue Community</span>}
                                  {open && <ExternalLink className="h-3 w-3 opacity-50 shrink-0 mt-0.5" />}
                                </button>
                              ) : (
                                <SidebarMenuSubButton asChild className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground !h-auto py-2">
                                  <NavLink to="/community" className="items-start">
                                    <Users className="mr-2 h-4 w-4 shrink-0 mt-0.5" />
                                    {open && <span className="flex-1 leading-tight">InnoTrue Community</span>}
                                    {open && <ExternalLink className="h-3 w-3 opacity-50 shrink-0 mt-0.5" />}
                                  </NavLink>
                                </SidebarMenuSubButton>
                              )}
                            ...
                            {talentLmsUser ? (
                              ...
                              <SidebarMenuSubButton asChild className="hover:bg-primary hover:text-primary-foreground text-sidebar-foreground !h-auto py-2">
                                <NavLink to="/academy" className="items-start">
                                  ...
                                  {open && <span className="flex-1 leading-tight">InnoTrue Academy</span>}
                                  {open && <ExternalLink className="h-3 w-3 opacity-50 shrink-0 mt-0.5" />}
                                </NavLink>
                              </SidebarMenuSubButton>
                            )}

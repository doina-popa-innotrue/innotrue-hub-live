...
import { FileText, ChevronRight, Clock, CheckCircle2, Send, Shield, Award, Play, Loader2 } from 'lucide-react';
import { useScenariosForModule, useScenarioAssignments, useScenarioProgress } from '@/hooks/useScenarios';
...
export function ModuleScenariosSection({ moduleId, enrollmentId }: ModuleScenariosSectionProps) {
  ...
  const { data: assignments, isLoading: assignmentsLoading, refetch: refetchAssignments } = useScenarioAssignments({ userId: user?.id });
...
function ScenarioItem({
  ...
  const statusConfig = {
    draft: { label: 'In Progress', variant: 'secondary' as const, icon: <Clock className="h-3 w-3" /> },
    submitted: { label: 'Submitted', variant: 'default' as const, icon: <Send className="h-3 w-3" /> },
    in_review: { label: 'Under Review', variant: 'outline' as const, icon: <Clock className="h-3 w-3" /> },
    evaluated: { label: 'Completed', variant: 'default' as const, icon: <CheckCircle2 className="h-3 w-3" /> },
  };
    ...
    <div className="border rounded-lg p-4 space-y-3">
      <div className="flex items-start justify-between gap-3">
        ...
        {hasAssignment ? (
          ...
          <Badge variant="outline" className="shrink-0">Not Started</Badge>
        )}
      </div>
      ...
      {hasAssignment && status === 'draft' && progress && progress.total > 0 && (
        <div className="space-y-1">
          <div className="flex justify-between text-xs text-muted-foreground">
            <span>Progress</span>
            <span>{progress.answered}/{progress.total} responses</span>
          </div>
      ...
      {/* Action button */}
      {hasAssignment ? (
        <Button asChild size="sm" variant={status === 'evaluated' ? 'outline' : 'default'} className="w-full sm:w-auto">

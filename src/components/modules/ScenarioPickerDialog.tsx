...
export function ScenarioPickerDialog({ excludeScenarioIds, onSelect, disabled, trigger }: ScenarioPickerDialogProps) {
  ...
  const filteredScenarios = useMemo(() => {
    return scenarios.filter(scenario => {
      ...
      if (searchQuery.trim()) {
        ...
        const matchesAssessment = scenario.capability_assessments?.name.toLowerCase().includes(query);
         const matchesCategory = scenario.scenario_categories?.name.toLowerCase().includes(query);
         if (!matchesTitle && !matchesDescription && !matchesAssessment && !matchesCategory) return false;
      }
                        ...
                        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                           {scenario.scenario_categories && (
                             <Badge 
                               ...
                               className="text-xs"
                               style={{
                                 borderColor: scenario.scenario_categories.color ? `var(--${scenario.scenario_categories.color}-500, currentColor)` : undefined
                               }}
                             >

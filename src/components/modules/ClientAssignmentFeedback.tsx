...
export function ClientAssignmentFeedback({ assignmentId }: ClientAssignmentFeedbackProps) {
  ...
  const questionNotesMap = new Map(
    snapshotData.questionNotes.map((n) => [n.question_id, n.content]),
  );
  ...
  const getPassFailStatus = (): { passed: boolean; label: string } | null => {
    if (
      !snapshotData?.assessment?.pass_fail_enabled ||
    ...
    <Card className="border-primary/30 bg-primary/5">
      <CardHeader>
        ...
        {scorerProfile && assignmentData.scored_at && (
          <CardDescription>
            Reviewed by {scorerProfile.name} on{" "}
            {new Date(assignmentData.scored_at).toLocaleDateString()}
          </CardDescription>
        )}
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {assignmentData.instructor_notes && (
          <div className="space-y-2">
            ...
            <RichTextDisplay
              content={assignmentData.instructor_notes}
              className="text-sm bg-background rounded-md p-3"
            />
          </div>
        )}
        ...
        <div className="bg-muted/50 rounded-lg p-4 space-y-3">
          ...
          <Button asChild className="w-full sm:w-auto">
            <Link
              to={`/capabilities/${snapshotData.snapshot.assessment_id}?snapshotId=${assignmentData.scoring_snapshot_id}`}
            >

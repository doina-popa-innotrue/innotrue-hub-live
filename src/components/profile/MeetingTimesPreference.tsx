...
export function MeetingTimesPreference({
  value,
  onChange,
  disabled,
  readOnly,
}: MeetingTimesPreferenceProps) {
  ...
  const updateAndNotify = useCallback(
    (newDayRanges: Map<string, TimeRange[]>) => {
  ...
  const addRange = (day: string) => {
    ...
    if (lastRange) {
      ...
      newEnd = `${Math.min(startHour + 2, 23)
        .toString()
        .padStart(2, "0")}:00`;
    }
  ...
  const updateRange = (day: string, index: number, field: "start" | "end", value: string) => {
    ...
    if (existing[index]) {
      ...
      if (field === "start" && existing[index].end <= value) {
        ...
        existing[index].end =
          `${endHour.toString().padStart(2, "0")}:${mins.toString().padStart(2, "0")}`;
      }

...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchOrganizationMembership = async (userId: string): Promise<OrganizationMembership | null> => {
    try {
  ...
  const fetchUserRolesAndMembership = async (userId: string) => {
    ...
    if (orgMembership && (orgMembership.role === 'org_admin' || orgMembership.role === 'org_manager')) {
  ...
  const signIn = async (email: string, password: string) => {
    ...
    if (!error && data.user) {
      ...
      if (roles.includes('admin')) {
        navigate('/admin');
      } else if (orgMembership && (orgMembership.role === 'org_admin' || orgMembership.role === 'org_manager')) {
        navigate('/org-admin');
  ...
  const switchRole = (role: UserRoleType) => {
    if (userRoles.includes(role)) {
      ...
      const isOnClientRoute = currentPath.startsWith('/dashboard') || 

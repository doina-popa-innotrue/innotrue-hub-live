...
export default function ScenarioAssignmentsManagement() {
  ...
  const pendingReview =
    assignments?.filter((a) => a.status === "submitted" || a.status === "in_review") || [];
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Assign scenarios to clients and evaluate their responses
          </p>
        </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {assignments?.filter((a) => a.status === "draft").length || 0}
                </p>
...
function AssignScenarioDialog() {
  ...
  const { data: enrollments } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Fetch program names
      const programIds = [
        ...new Set(data.map((e) => e.program_id).filter((id): id is string => id != null)),
      ];
      ...
      const userIds = [
        ...new Set(data.map((e) => e.client_user_id).filter((id): id is string => !!id)),
      ];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (assignmentMode === "single") {
      if (!formData.template_id || !formData.user_id) {
        toast({
          title: "Error",
          description: "Please select template and client",
          variant: "destructive",
        });
        return;
      }
      ...
      if (!formData.template_id || selectedUserIds.length === 0) {
        toast({
          title: "Error",
          description: "Please select template and at least one client",
          variant: "destructive",
        });
        return;
      }
      ...
      const selectedEnrollments =
        enrollments?.filter(
          (e) => e.client_user_id && selectedUserIds.includes(e.client_user_id),
        ) || [];
  ...
  const selectAllInProgram = (programId: string) => {
    ...
    const userIdsInProgram = programEnrollments
    ...
    >
      ...
      <DialogContent className="max-w-lg">
        ...
        <Tabs
          ...
          onValueChange={(v) => setAssignmentMode(v as "single" | "bulk")}
        >
          ...
          <form onSubmit={handleSubmit} className="mt-4 space-y-4">
            ...
            <TabsContent value="bulk" className="mt-0 space-y-4">
              <div>
                ...
                <ScrollArea className="h-[200px] border rounded-md p-2 mt-1">
                  {Array.from(enrollmentsByProgram.entries()).map(
                          ...
                          <div className="space-y-1 pl-2">
                            {programEnrollments.map((enrollment) => {
                              ...
                              const isSelected = selectedUserIds.includes(
                                enrollment.client_user_id,
                              );
                              return (
                                <div key={enrollment.id} className="flex items-center gap-2 py-1">
                                  <Checkbox
                                    id={enrollment.id}
                                    checked={isSelected}
                                    onCheckedChange={() =>
                                      toggleUserSelection(enrollment.client_user_id!)
                                    }
                                  />
                                  <label
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              >
                ...
                {assignmentMode === "bulk"
                  ? `Assign to ${selectedUserIds.length} Clients`
                  : "Assign"}
              </Button>
            </div>
          </form>

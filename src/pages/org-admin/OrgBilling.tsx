...
export default function OrgBilling() {
  ...
  const [seatUsage, setSeatUsage] = useState<{ used: number; max: number | null }>({ used: 0, max: null });
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Manage your organization's subscription and credits
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Members with org-sponsored access
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="text-2xl font-bold">
                {formatCredits(summary?.expiring_soon ?? 0)}
              </div>
            )}
            <p className="text-xs text-muted-foreground mt-1">
        ...
        <>
          {seatUsage.used >= seatUsage.max && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                You've used all {seatUsage.max} sponsored seats included in your plan. 
                Upgrade your plan to add more members with org-sponsored access.
              </AlertDescription>
            </Alert>
          )}
          {seatUsage.used >= seatUsage.max * 0.8 && seatUsage.used < seatUsage.max && (
            <Alert variant="default" className="border-warning/50 bg-warning/10">
              ...
              <AlertDescription>
                You're using {seatUsage.used} of {seatUsage.max} sponsored seats ({Math.round((seatUsage.used / seatUsage.max) * 100)}%). 
        ...
        <TabsContent value="credits" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Top up your organization's credit balance
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                          ...
                          {isDowngrade && (
                            <Alert variant="destructive" className="mt-4">
                              ...
                              <AlertDescription className="text-xs">
                                You have {seatUsage.used} sponsored seats, but this plan only allows {tierMaxSeats}. 
                                Remove {seatUsage.used - tierMaxSeats!} sponsored seats before switching.
                              </AlertDescription>
                            </Alert>
                          )}
                        </CardContent>
                        <CardFooter>
                          <Button 
                            ...
                            onClick={() => {
                              ...
                              subscribePlatform({ tierId: tier.id, billingPeriod: selectedBillingPeriod });
                            }}
                            disabled={isSubscribing || isCurrentTier || !canManageBilling}
                          >
                            {isCurrentTier ? 'Current Plan' : isSubscribing ? 'Processing...' : isDowngrade ? 'Exceeds Seat Limit' : 'Subscribe'}
                          </Button>
                        </CardFooter>
                      </Card>
                    );

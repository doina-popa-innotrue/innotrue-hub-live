...
export default function DecisionAnalytics() {
  ...
  const valuesStats = (decisions?.flatMap((d) => (d.decision_values as any[]) || []) ?? [])
    .reduce(
      (acc, v) => {
        const existing = acc.find(
          (item: { name: string; count: number; totalScore: number; avgScore: number }) =>
            item.name === v.value_name,
        );
        if (existing) {
          existing.count++;
  ...
  const avgConfidence =
    (decisions?.reduce((sum, d) => sum + (d.confidence_level || 0), 0) ?? 0) /
    (decisions?.length || 1);

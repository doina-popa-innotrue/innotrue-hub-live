...
export default function DecisionFollowUps() {
  ...
  const overdueReminders = pendingReminders.filter(r => isPast(new Date(r.reminder_date)) && !isToday(new Date(r.reminder_date)));
  const todayReminders = pendingReminders.filter(r => isToday(new Date(r.reminder_date)));
  const upcomingReminders = pendingReminders.filter(r => isFuture(new Date(r.reminder_date)) && !isToday(new Date(r.reminder_date)));

...
import { ArrowLeft, CheckCircle2, Circle, Clock, ExternalLink, Timer, Award, ChevronRight, FileText, Link as LinkIcon, Paperclip, Plus, RotateCcw, PlayCircle } from 'lucide-react';
...
export default function ModuleDetail() {
  ...
  const { checkModulePlanAccess, getPlanNameForTier, userPlan, isLoading: planAccessLoading } = usePlanAccess();
  ...
  if (loading) return <SessionMismatchGuard><div className="p-6">Loading...</div></SessionMismatchGuard>;
  if (!module) return <SessionMismatchGuard><div className="p-6">Module not found</div></SessionMismatchGuard>;
  ...
  return (
    ...
    <div className="space-y-6">
        ...
        {!isLocked && (
          <div className="flex flex-wrap items-center gap-2">
            <Button
              ...
              size="sm"
            >
              {module.progress?.status === 'completed' ? 'Mark as Incomplete' : 'Mark as Complete'}
            </Button>
      ...
      {/* New module sections */}
      {!isLocked && (
        <ModuleSectionsDisplay moduleId={module.id} />
      )}
      ...
      {!isLocked && module.links && module.links.length > 0 && (
        <Card className={module.module_type === 'content' ? 'border-primary/50 bg-primary/5' : ''}>
          <CardHeader>
            <CardTitle className={module.module_type === 'content' ? 'flex items-center gap-2' : ''}>
              {module.module_type === 'content' && (
                <Award className="h-5 w-5 text-primary" />
              )}
              {module.module_type === 'content' ? 'InnoTrue Academy Learning Module' : 'External Links'}
            </CardTitle>
            {module.module_type === 'content' && (
              <CardDescription>
            ...
            <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2">
              {module.links.map((link, index) => (
                <Button
                  ...
                  variant={module.module_type === 'content' && link.type === 'talentlms' ? 'default' : 'outline'}
        ...
        <Card>
          <CardHeader>
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
              ...
              {!editingNotes && (
                <Button onClick={() => setEditingNotes(true)} size="sm" className="shrink-0 w-full sm:w-auto">

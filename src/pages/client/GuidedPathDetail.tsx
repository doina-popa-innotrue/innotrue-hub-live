...
import { 
  Map, Target, Flag, CheckSquare, ChevronDown, ChevronRight, 
  Clock, Copy, BookOpen, Calendar 
} from 'lucide-react';
...
const VALID_GOAL_CATEGORIES: GoalCategory[] = [
  'family_home', 'financial_career', 'mental_educational', 'spiritual_ethical',
  'social_cultural', 'physical_health', 'health_fitness', 'career_business',
  'finances', 'relationships', 'personal_growth', 'fun_recreation',
  'physical_environment', 'family_friends', 'romance', 'contribution'
];
...
export default function GuidedPathDetail() {
  ...
  const copyMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: 'Path Copied!',
        description: 'Your goals, milestones, and tasks have been created. Visit your Goals page to see them.',
      });
  ...
  const totalMilestones = template.guided_path_template_goals.reduce(
    (sum, g) => sum + g.guided_path_template_milestones.length, 0
  );
  const totalTasks = template.guided_path_template_goals.reduce(
    (sum, g) => sum + g.guided_path_template_milestones.reduce(
      (mSum, m) => mSum + m.guided_path_template_tasks.length, 0
    ), 0
  );
                              ...
                              <div className="border-t px-3 py-2 bg-muted/20 space-y-1">
                                ...
                                {milestone.guided_path_template_tasks.length === 0 ? (
                                  ...
                                  milestone.guided_path_template_tasks.map((task) => (
                                    <div key={task.id} className="flex items-center gap-2 p-2 border rounded bg-background">
      ...
      <Dialog open={copyDialogOpen} onOpenChange={setCopyDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will create {totalGoals} goal{totalGoals !== 1 ? 's' : ''}, {totalMilestones} milestone{totalMilestones !== 1 ? 's' : ''}, and {totalTasks} task{totalTasks !== 1 ? 's' : ''} for you to personalize.
            </DialogDescription>
          </DialogHeader>
              ...
              >
                <div className="flex items-center space-x-2 p-2 rounded border hover:bg-muted/50 cursor-pointer">
                  ...
                  <Label htmlFor="pace-min" className="cursor-pointer flex-1">
                    ...
                    <span className="text-xs text-muted-foreground ml-2">Minimum recommended time</span>
                  </Label>
                </div>
                <div className="flex items-center space-x-2 p-2 rounded border hover:bg-muted/50 cursor-pointer">
                  ...
                  <Label htmlFor="pace-optimal" className="cursor-pointer flex-1">
                    ...
                    <span className="text-xs text-muted-foreground ml-2">Optimal pace (recommended)</span>
                  </Label>
                </div>
                <div className="flex items-center space-x-2 p-2 rounded border hover:bg-muted/50 cursor-pointer">
                  ...
                  <Label htmlFor="pace-max" className="cursor-pointer flex-1">
                    ...
                    <span className="text-xs text-muted-foreground ml-2">Maximum time per milestone</span>
                  </Label>
                </div>
              </RadioGroup>

...
export default function ExplorePrograms() {
  ...
  const [programAccessMap, setProgramAccessMap] = useState<
    Map<
      string,
      {
        isLocked: boolean;
    ...
    >
  >(new Map());
  const [previewCrossCompletions, setPreviewCrossCompletions] =
    useState<ProgramCompletionInfo | null>(null);
  const [interestCrossCompletions, setInterestCrossCompletions] =
    useState<ProgramCompletionInfo | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchPrograms = async () => {
      ...
      const enrollmentsByProgram = new Map<
        string,
        { status: string; enrollment_number: number }[]
      >();
      ...
      completedProgress?.forEach((progress: any) => {
        ...
        programCompletionCounts.set(programId, (programCompletionCounts.get(programId) || 0) + 1);
      ...
      for (const program of programsData || []) {
        ...
        const completedInSameCategory = Array.from(enrollmentsByProgram.entries()).filter(
          ([pid, enrollments]) => {
        ...
        const activeInSameCategory = Array.from(enrollmentsByProgram.entries()).filter(
          ([pid, enrollments]) => {
            const prog = programsData.find((p) => p.id === pid);
            return (
              prog?.category === program.category && enrollments.some((e) => e.status === "active")
            );
          },
        ).length;
      ...
      const enrichedPrograms: Program[] = (programsData || []).map((program) => {
        ...
        return {
          ...
          isRecommended:
            (recommendationScores.get(program.id) || 0) > 30 && programEnrollments.length === 0,
  ...
  const handleToggleFavorite = async (programId: string) => {
    ...
    try {
      ...
      // Update local state
      setPrograms((prev) =>
        prev.map((p) => (p.id === programId ? { ...p, isFavorite: !p.isFavorite } : p)),
      );
    ...
    try {
      ...
      if (isScheduledDate && selectedSchedule) {
        ...
        if (capacity > 0 && enrolledCount >= capacity) {
          ...
          const { error: insertError } = await supabase.from("program_waitlist").insert({
            user_id: user.id,
        ...
        const updatedSchedules = scheduledDates.map((s) =>
          s.id === selection ? { ...s, enrolled_count: (s.enrolled_count || 0) + 1 } : s,
        );
      ...
      const { error } = await supabase.from("program_interest_registrations").insert(insertData);
      ...
      // Update local state
      setPrograms((prev) =>
        prev.map((p) => (p.id === interestProgram.id ? { ...p, hasInterest: true } : p)),
      );
      ...
      if (error.code === "23505") {
        toast({
          title: "You've already expressed interest in this program",
          variant: "destructive",
        });
      } else {
  ...
  const ProgramCard = ({ program }: { program: Program }) => {
    ...
    const requiredPlanName = accessInfo?.requiredTier
      ? getPlanNameForTier(accessInfo.requiredTier)
      : "";
        ...
        className={`relative transition-all ${
          program.isCompleted
            ...
            : isPlanLocked
              ...
              : "hover:shadow-lg"
        } ${program.isEnrolled && !program.isCompleted ? "ring-2 ring-primary/50" : ""}`}
      >
        ...
        <CardHeader>
          <div className="flex items-start justify-between gap-2 pr-12">
            <div className="flex-1">
              <CardTitle className="line-clamp-2 mb-2">{program.name}</CardTitle>
        ...
        <CardContent className="space-y-2">
          ...
          {isPlanLocked ? (
            ...
            <div className="space-y-2">
              ...
              <Button className="w-full" onClick={() => setInterestProgram(program)}>
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="all" className="space-y-6">
        <TabsList>
          ...
          <TabsTrigger value="favorites">My Favorites ({favoritePrograms.length})</TabsTrigger>
        </TabsList>

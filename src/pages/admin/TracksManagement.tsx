...
export default function TracksManagement() {
  ...
  const { data: trackFeatures } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("track_features").select("*");
      if (error) throw error;
  ...
  const cloneTrackMutation = useMutation({
    mutationFn: async (sourceTrack: Track) => {
      ...
      if (sourceFeatures && sourceFeatures.length > 0) {
        ...
        const { error: insertError } = await supabase.from("track_features").insert(newFeatures);
  ...
  const getTrackFeatureStatus = (trackId: string, featureId: string) => {
    return trackFeatures?.find((tf) => tf.track_id === trackId && tf.feature_id === featureId);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Manage tracks and their feature entitlements</p>
        </div>
                ...
                <TableBody>
                  {tracks?.map((track) => (
                    <TableRow key={track.id}>
                      ...
                      <TableCell>
                        <code className="rounded bg-muted px-2 py-1 text-sm">{track.key}</code>
                      </TableCell>
        ...
        <TabsContent value="features">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure which features and limits each track provides. When users have multiple
                tracks, highest limit wins.
                        ...
                        <table className="w-full caption-bottom text-sm">
                          <thead className="sticky top-0 z-10 [&_tr]:border-b">
                            <tr className="border-b">
                              <th className="h-10 px-4 text-left align-middle font-medium text-muted-foreground bg-muted/50">
                                Feature
                              </th>
                              {tracks
                                ...
                                .map((track) => (
                                  <th
                                    key={track.id}
                                    className="h-10 px-4 text-center align-middle font-medium text-muted-foreground bg-muted/50 min-w-[120px]"
                                  >
                                    {track.name}
                                  </th>
                                ))}
        ...
        <DialogContent>
          <form onSubmit={handleSubmit}>
            <DialogHeader>
              ...
              <DialogDescription>
                {editingTrack ? "Update track details" : "Add a new track to the system"}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setTrackForm({ ...trackForm, display_name: e.target.value })}
            ...
            <DialogFooter>
              ...
              <Button type="submit">{editingTrack ? "Update" : "Create"} Track</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>

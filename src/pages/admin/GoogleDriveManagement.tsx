...
export default function GoogleDriveManagement() {
  ...
  const fetchData = async () => {
    ...
    // Fetch all mappings
    const { data: mappingsData } = await supabase.from("google_drive_users").select("*");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingMapping) {
        ...
        const { error } = await supabase.from("google_drive_users").insert({
  ...
  const handleDelete = async (id: string) => {
    ...
    const { error } = await supabase.from("google_drive_users").delete().eq("id", id);
  ...
  const unmappedUsers = profiles.filter((p) => !mappings.find((m) => m.user_id === p.id));
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage Google Drive folder assignments for users</p>
        </div>
        <Dialog
          open={isDialogOpen}
          onOpenChange={(open) => {
        ...
        >
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{editingMapping ? "Edit" : "Add"} Google Drive Mapping</DialogTitle>
            </DialogHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Users with assigned Google Drive folders</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (

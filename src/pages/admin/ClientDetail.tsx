...
import { ExternalLink, Award, Timer, RefreshCw, UserPlus, X, FolderOpen, Save, Coins, AlertTriangle, PauseCircle, PlayCircle } from 'lucide-react';
...
export default function ClientDetail() {
  ...
  const [programPlans, setProgramPlans] = useState<{ id: string; name: string; tier_level: number }[]>([]);
  ...
  useEffect(() => {
    async function fetchData() {
      const { data: profile } = await supabase.from('profiles').select('*, plans(id, name, key)').eq('id', id).single();
      const { data: clientProfile } = await supabase.from('client_profiles').select('*').eq('user_id', id).maybeSingle();
      const { data: enrollmentData } = await supabase.from('client_enrollments').select('*, programs(*), program_plans(id, name, tier_level)').eq('client_user_id', id);
      const { data: allPrograms } = await supabase.from('programs').select('*, program_plans(id, name, tier_level)').eq('is_active', true);
      const { data: plans } = await supabase.from('plans').select('id, name, key').eq('is_active', true);
      const { data: programPlansData } = await supabase.from('program_plans').select('id, name, tier_level').eq('is_active', true).order('tier_level');
  ...
  async function assignProgram() {
    ...
    const validDiscount = discountPercent && discountPercent > 0 && discountPercent <= 100 ? discountPercent : null;
    ...
    if (finalCreditCost && finalCreditCost > 0) {
      const { data: consumeResult, error: consumeError } = await supabase.rpc('consume_credits_fifo', {
        p_owner_type: 'user',
        p_owner_id: id,
  ...
  async function loadEnrollmentModules(enrollmentId: string, programId: string, forceRefresh: boolean = false) {
  ...
  async function updateEnrollmentTier(enrollmentId: string, newTier: string, oldTier: string, programId: string, programName: string) {
  ...
  async function toggleEnrollmentStatus(enrollmentId: string, currentStatus: string, programName: string) {
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="pt-2 border-t space-y-4">
              <p><strong>Current Status:</strong> <Badge variant={isActive ? 'default' : 'secondary'}>{client.status || 'active'}</Badge></p>
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    {statusMarkers.map((marker) => (
                      <SelectItem key={marker.id} value={marker.name}>{marker.name}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                {statusMarker && (
                  <Badge variant="secondary">{statusMarker}</Badge>
                )}
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                {client.plans && (
                  <Badge variant="secondary">{client.plans.key}</Badge>
                )}
              </div>
                  ...
                  <Dialog open={coachDialogOpen} onOpenChange={setCoachDialogOpen}>
                    ...
                    <DialogContent>
                      ...
                      <div className="space-y-4">
                        ...
                        <Button onClick={assignCoach} disabled={!selectedCoach}>Assign</Button>
                      </div>
                    </DialogContent>
                  </Dialog>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {instructors.length > 0 ? (
                    instructors.map((instructor) => (
                      <Badge key={instructor.id} variant="outline" className="flex items-center gap-1">
                        {instructor.name}
                        <button 
                  ...
                  <Dialog open={instructorDialogOpen} onOpenChange={setInstructorDialogOpen}>
                    ...
                    <DialogContent>
                      ...
                      <div className="space-y-4">
                          ...
                          <Select value={selectedInstructor} onValueChange={setSelectedInstructor}>
                            ...
                            <SelectContent>
                              {availableInstructors
                                .filter(i => !instructors.find(assigned => assigned.id === i.id))
                                .map((instructor) => (
                        ...
                        <Button onClick={assignInstructor} disabled={!selectedInstructor}>Assign</Button>
                      </div>
                    </DialogContent>
                  </Dialog>
                </div>
              ...
              <p><strong>Notes:</strong> {client.notes || 'No notes'}</p>
            </div>
          </div>
        </CardContent>
      </Card>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={selectedTier} onValueChange={setSelectedTier}>
                    ...
                    <SelectContent>
                      {(programs.find(p => p.id === selectedProgram)?.tiers as string[] || ['Essentials', 'Premium']).map((tier) => (
                ...
                <div className="space-y-2">
                  ...
                  <Select value={selectedProgramPlan || 'default'} onValueChange={(val) => setSelectedProgramPlan(val === 'default' ? '' : val)}>
                    <SelectTrigger>
                      ...
                      {adminDiscountPercent && parseFloat(adminDiscountPercent) > 0 && (
                        <>
                          <div className="flex justify-between text-success">
                            ...
                            <span>-{Math.round(tierCreditCost * parseFloat(adminDiscountPercent) / 100).toLocaleString()} credits</span>
                          </div>
                          <div className="flex justify-between font-medium border-t pt-1">
                            <span>Final cost:</span>
                            <span className="text-primary">
                              {Math.round(tierCreditCost * (1 - parseFloat(adminDiscountPercent) / 100)).toLocaleString()} credits
                            </span>
                          </div>
                        </>
                      )}
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0 flex-1">
                    ...
                    <CardDescription className="mt-1">
                      Status: <Badge variant={enrollment.status === 'active' ? 'default' : 'secondary'}>
                        {enrollment.status}
                      </Badge>
                    </CardDescription>
                  </div>
                  <div className="flex flex-wrap items-center gap-3">
                    <div className="flex items-center gap-2">
                      <Label className="text-sm text-muted-foreground whitespace-nowrap">Tier:</Label>
                      <Select
                        value={enrollment.tier}
                        onValueChange={(value) => updateEnrollmentTier(
                          enrollment.id, 
                          value, 
                      ...
                      >
                        ...
                        <SelectContent>
                          {((enrollment.programs?.tiers as string[]) || ['Essentials', 'Premium']).map((tier) => (
                            <SelectItem key={tier} value={tier.toLowerCase()}>
                              {tier}
                            </SelectItem>
                          ))}
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  {enrollment.status !== 'completed' && (
                    <Button
                      ...
                      size="sm"
                      className={enrollment.status === 'paused' ? 'bg-green-600 hover:bg-green-700' : ''}
                    >
                      ...
                      return (
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex-1">
                              ...
                              <RichTextDisplay content={module.description ?? ''} className="text-sm text-muted-foreground mt-1" />
                            ...
                            <div className="flex items-center gap-2">
                              <Badge variant={
                                module.progress?.status === 'completed' ? 'default' :
                                module.progress?.status === 'in_progress' ? 'secondary' :
                                'outline'
                              }>
                                ...
                                <ManualCompletionControls
                                  ...
                                  onSuccess={() => {
                                    loadEnrollmentModules(enrollment.id, enrollment.programs.id, true);
                                  }}
                                />
                            </div>
                          </div>
                            ...
                            <div className="mt-3 rounded-lg border bg-muted/50 p-3 space-y-2">
                              ...
                              <div className="grid grid-cols-2 gap-3 text-sm">
                                <div>
                                  ...
                                  <div className="font-medium">{tlmsProgress.completion_status.replace('_', ' ')}</div>
                                </div>
                                <div>
                                  <span className="text-muted-foreground">Progress:</span>
                                  <div className="font-medium">{tlmsProgress.progress_percentage}%</div>
                                </div>
                                {tlmsProgress.time_spent_minutes > 0 && (
                                  <div>
                                    ...
                                    <div className="font-medium">
                                      {Math.floor(tlmsProgress.time_spent_minutes / 60)}h {tlmsProgress.time_spent_minutes % 60}m
                                    </div>
                                  </div>
                                )}
                              ...
                              {tlmsProgress.completed_at && (
                                <div className="text-xs text-muted-foreground pt-2 border-t">
                                  Completed: {new Date(tlmsProgress.completed_at).toLocaleDateString()}
                                </div>
                              )}
                            </div>
                          )}

...
export default function ProgramsList() {
  ...
  const [scheduledDates, setScheduledDates] = useState<Array<{ id: string; date: string; title: string; capacity: number; enrolled_count: number }>>([]);
  ...
  async function cloneProgram(program: Program) {
    try {
      ...
      toast.success(`Program cloned successfully! Created "${newName}" with ${modules?.length || 0} modules.`);
      fetchPrograms();
    } catch (error: any) {
  ...
  async function handleDeleteCategory(categoryId: string) {
    if (!confirm('Are you sure? Programs using this category will keep their current value.')) return;
    try {
      const { error } = await supabase
        .from('program_categories')
        .delete()
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      setScheduledDates([...scheduledDates, { id: crypto.randomUUID(), date: '', title: '', capacity: 0, enrolled_count: 0 }]);
                    }}
                  >
                    <Plus className="mr-2 h-4 w-4" />
                    Add Scheduled Date
                  ...
                  <TableCell>
                    <div className="flex items-center gap-3">
                      <Avatar className="h-8 w-8">
                        ...
                        <AvatarFallback className="text-xs">{program.name.slice(0, 2).toUpperCase()}</AvatarFallback>
                      </Avatar>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setCategoryForm({ ...categoryForm, key: e.target.value.toLowerCase().replace(/\s+/g, '-') })}

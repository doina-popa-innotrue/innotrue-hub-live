...
export default function CalcomMappingsManagement() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const submitData = {
      ...
      default_group_id: formData.session_target === 'group_session' ? (formData.default_group_id || null) : null,
      default_module_id: formData.session_target === 'module_session' ? (formData.default_module_id || null) : null,
  ...
  const columns: AdminTableColumn<CalcomMapping>[] = [
    ...
    },
    {
      key: 'scheduling_url',
      header: 'Booking URL',
      accessor: (item) => item.scheduling_url ? (
        <a 
    ...
    },
    {
      ...
      header: 'Status',
      accessor: (item) => (
        <AdminStatusBadge isActive={item.is_active} />
      ),
    },
  ];
  ...
  const formContent = (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        />
        <p className="text-xs text-muted-foreground">
          Friendly name shown in admin lists
        </p>
      </div>
      ...
      <div className="space-y-2">
        ...
        />
        <p className="text-xs text-muted-foreground">
          Override URL for client self-booking. Leave empty to use Event Type ID for all bookings via API.
        </p>
      </div>
      ...
      {formData.session_target === 'module_session' && (
        <>
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              <strong>Leave empty</strong> for module types shared across multiple programs. 
              The system uses metadata from booking URLs to link sessions to the correct enrollment. 
          ...
          {formData.default_program_id && (
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setFormData({ ...formData, default_module_id: value === '__none__' ? '' : value })}
              >
                <SelectTrigger>
      ...
      {formData.session_target === 'group_session' && (
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) => setFormData({ ...formData, default_group_id: value === '__none__' ? '' : value })}
          >
            <SelectTrigger>
      ...
      <Tabs defaultValue="event-mappings" className="space-y-6">
        ...
        <TabsContent value="event-mappings" className="space-y-6">
          <div className="rounded-lg border p-4 bg-muted/30 text-sm">
            <p className="text-muted-foreground">
              <strong className="text-foreground">Purpose:</strong> Event Type Mappings handle <strong>incoming Cal.com webhooks</strong>. 
              When a booking is created in Cal.com, these mappings determine whether to create a module session or group session, 
              and can set default program/group associations. The Module Type field links this event to the correct session category.
            </p>
          </div>
          ...
          <AdminTable
            ...
            renderActions={(mapping) => (
              <AdminTableActions
                ...
                onDelete={() => handleDelete(mapping.id, `Delete mapping for "${mapping.calcom_event_type_name || mapping.calcom_event_type_id}"?`)}
              />
            )}
            emptyState={{
              icon: Calendar,

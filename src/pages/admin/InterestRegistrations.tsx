...
export default function InterestRegistrations() {
      ...
      const registrationsWithDetails = await Promise.all(
        (data || []).map(async (reg: any) => {
          ...
          const { data: profileData } = (await supabase
            ...
            .eq("id", reg.user_id)
            .single()) as {
            data: { name: string; username: string | null; is_disabled: boolean } | null;
          };

          return {
            ...reg,
            profiles: profileData
              ? { name: profileData.name, is_disabled: profileData.is_disabled }
              : null,
  ...
  async function bulkEnroll() {
    ...
    try {
      ...
      for (const reg of selectedRegs) {
        ...
        if (
          reg.enrollment_timeframe === "scheduled" &&
          reg.scheduled_date_id &&
          reg.programs?.scheduled_dates
        ) {
  ...
  const getTimeframeDisplay = (reg: Registration) => {
    if (
      ...
      const schedule = reg.programs.scheduled_dates.find(
        (s: any) => s.id === reg.scheduled_date_id,
      );
      if (schedule) {
    ...
    switch (reg.enrollment_timeframe) {
      ...
      case "1-3_months":
        return "1-3 Months";
      default:
        return reg.enrollment_timeframe;
    }
  };
              ...
              {filteredRegistrations.length === 0 ? (
                ...
                filteredRegistrations.map((reg) => (
                  <TableRow key={reg.id}>
                    ...
                    <TableCell className="font-medium">{reg.profiles?.name || "Unknown"}</TableCell>
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent>
                              <p className="font-medium mb-1">
                                {(reg.completed_modules_elsewhere as any[])?.length || 0} modules
                                completed elsewhere
                              </p>
                              <ul className="text-xs space-y-0.5">
                                {(reg.completed_modules_elsewhere as any[])
                                  ?.slice(0, 5)
                                  .map((m: any, i: number) => (
                                    <li key={i}>
                                      • {m.moduleTitle} ({m.completedInProgram})
                                    </li>
                                  ))}
                                {((reg.completed_modules_elsewhere as any[])?.length || 0) > 5 && (
                                  <li>
                                    ...and {(reg.completed_modules_elsewhere as any[]).length - 5}{" "}
                                    more
                                  </li>
                                )}
                              </ul>

    ...
    <div className="space-y-6 p-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Assign add-on packages to individual users</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Users with add-ons assigned beyond their base plan</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              ...
              {userAddOns?.map((ua) => {
                ...
                return (
                  <TableRow key={ua.id}>
                      ...
                      {isConsumable ? (
                        <div className="space-y-1 min-w-[120px]">
                          <div className="flex items-center justify-between text-xs">
                            <span>
                              {ua.quantity_remaining || 0} / {ua.quantity_granted || 0}
                            </span>
                    ...
                    <TableCell>{format(new Date(ua.granted_at), "MMM d, yyyy")}</TableCell>
                    <TableCell>
                      {ua.expires_at ? (
                        <div className="flex items-center gap-1">
                          ...
                          {isExpired(ua.expires_at) && <Badge variant="destructive">Expired</Badge>}
                        </div>
                      ) : (
                        <Badge variant="secondary">No expiration</Badge>
                      )}
                    </TableCell>
                    <TableCell className="max-w-[150px] truncate">{ua.notes || "-"}</TableCell>
                    <TableCell>
          ...
          <div className="space-y-4">
            ...
            {selectedAddOn?.is_consumable && (
              <div className="space-y-2 p-3 border rounded-md bg-amber-50 dark:bg-amber-950/20">
                <Label htmlFor="custom_quantity">Credits to Grant</Label>
                <Input
                  id="custom_quantity"
                  type="number"
                  value={formData.custom_quantity}
                  onChange={(e) => setFormData({ ...formData, custom_quantity: e.target.value })}

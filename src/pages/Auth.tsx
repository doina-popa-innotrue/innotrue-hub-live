...
import { z } from 'zod';
import { Eye, EyeOff, Target, Users, TrendingUp, Sparkles, LucideIcon, KeyRound } from 'lucide-react';
...
export default function Auth() {
  ...
  const [loginEmail, setLoginEmail] = useState(loginHintParam ? decodeURIComponent(loginHintParam) : '');
  ...
  // Default to login for specialized auth pages (standard UX)
  // Only show signup first if explicitly configured via default_to_signup or legacy wheel flow
  const defaultToSignup = wheelCompleted; // Only legacy wheel flow triggers signup-first
  const [activeTab, setActiveTab] = useState<'login' | 'signup'>(defaultToSignup ? 'signup' : 'login');
  
  // Update tab when context loads - only if explicitly configured
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen font-sans">
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center p-6 sm:p-8 lg:p-16 bg-background flex-1">
        ...
        {(resetMode && isRecoverySession) ? renderPasswordResetForm() : (
          <div className="max-w-md mx-auto w-full">

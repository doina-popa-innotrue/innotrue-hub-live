-- Fix get_user_credit_summary_v2 to use correct credit_batches column names
...
BEGIN
  ...
  -- Get program credits from credit_batches (using correct column names)
  SELECT 
  ...
  -- Get bonus credits (add-ons, purchases, grants) using correct column names
  SELECT COALESCE(SUM(cb.remaining_amount), 0)
  INTO v_bonus_credits
  FROM credit_batches cb
  WHERE cb.owner_type = 'user'
    AND cb.owner_id = p_user_id
    AND cb.source_type IN ('grant', 'purchase', 'addon', 'rollover', 'subscription')
    AND cb.is_expired = false
    AND (cb.expires_at IS NULL OR cb.expires_at > now());
...
-- Fix consume_credits_fifo to use correct column names and support org consumption
  ...
  IF v_remaining_to_consume > 0 THEN
    FOR v_batch IN
      ...
      WHERE owner_type = p_owner_type
        AND owner_id = v_effective_owner_id
        AND source_type IN ('grant', 'purchase', 'addon', 'rollover', 'subscription')
        AND is_expired = false
        AND (expires_at IS NULL OR expires_at > now())
        AND remaining_amount > 0
      ORDER BY expires_at NULLS LAST, created_at

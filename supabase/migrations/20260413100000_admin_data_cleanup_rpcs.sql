-- Admin data cleanup RPCs for ongoing test data removal
-- Supports: scenario_assignments, capability_snapshots, module_assignments, module_progress

-- =============================================================================
-- PREVIEW RPC — returns counts of what would be deleted
-- =============================================================================
...
-- =============================================================================
-- EXECUTE RPC — performs the actual deletion
-- =============================================================================
...
BEGIN
  ...
  WHEN 'scenario_assignments' THEN
    ...
    -- 3. Delete (CASCADE handles paragraph_responses, paragraph_evaluations, paragraph_question_scores)
    WITH deleted AS (
  ...
  WHEN 'capability_snapshots' THEN
    ...
    -- 3. Explicit DELETE on goal_assessment_links (non-CASCADE FK)
    ...
    -- 4. Delete (CASCADE handles ratings, domain_notes, question_notes, instructor_evals, dev_item links)
    WITH deleted AS (
...
-- Grant execute to authenticated users (SECURITY DEFINER + has_role guard handles auth)
GRANT EXECUTE ON FUNCTION admin_data_cleanup_preview TO authenticated;
GRANT EXECUTE ON FUNCTION admin_data_cleanup_execute TO authenticated;

-- Create table for excluded users from analytics
...
-- Only admins can view/manage exclusions
CREATE POLICY "Admins can view excluded users"
  ON public.analytics_excluded_users
  FOR SELECT
  USING (public.has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Admins can insert excluded users"
  ON public.analytics_excluded_users
  FOR INSERT
  WITH CHECK (public.has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Admins can delete excluded users"
  ON public.analytics_excluded_users
  FOR DELETE
  USING (public.has_role(auth.uid(), 'admin'::app_role));

-- Create function to get aggregated analytics (anonymized)

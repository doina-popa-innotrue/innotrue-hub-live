...
-- =============================================================================
-- SECTION 9b: ASSESSMENT CATEGORIES & CAPABILITY ASSESSMENTS
-- =============================================================================

-- Assessment categories
INSERT INTO public.assessment_categories (name, description, order_index, is_active) VALUES
  ('capability', 'Capability assessments for technical and professional skills', 1, true),
  ('leadership', 'Leadership and management assessments', 2, true),
  ('values', 'Values and alignment assessments', 3, true),
  ('personality', 'Personality trait assessments', 4, true),
  ('aptitude', 'Aptitude and ability assessments', 5, true),
  ('career', 'Career planning and direction assessments', 6, true),
  ('emotional-intelligence', 'Emotional intelligence assessments', 7, true),
  ('team', 'Team dynamics and collaboration assessments', 8, true),
  ('architecture', 'Architecture and technical design assessments', 9, true),
  ('psychometric', 'Psychometric and behavioural assessments', 10, true),
  ('other', 'Other assessment types', 11, true)
ON CONFLICT (name) DO NOTHING;

-- Architecture Self Knowledge Check (7 domains, 49 questions)
DO $$
DECLARE
  v_assessment_id UUID;
  v_domain_id UUID;
  v_arch_category_id UUID;
BEGIN
  -- Get architecture category
  SELECT id INTO v_arch_category_id FROM public.assessment_categories WHERE name = 'architecture' LIMIT 1;

  -- Skip if assessment already exists
  IF EXISTS (SELECT 1 FROM public.capability_assessments WHERE slug = 'architecture-self-knowledge-check') THEN
    RETURN;
  END IF;

  -- Create the assessment
  INSERT INTO public.capability_assessments (
    name, slug, description, instructions, rating_scale, is_active, is_public, category_id
  ) VALUES (
    ...
    5, true, false, v_arch_category_id
  ...
  -- Domain 2: Security & Identity Management (7 questions)
  INSERT INTO public.capability_domains (assessment_id, name, description, order_index)
  ...
  -- Domain 3: Data Management & Architecture (7 questions)
  INSERT INTO public.capability_domains (assessment_id, name, description, order_index)
  ...
  -- Domain 4: Integration Patterns, Capabilities & Strategies (6 questions)
  INSERT INTO public.capability_domains (assessment_id, name, description, order_index)
  ...
  -- Domain 5: Governance, Development Lifecycle & Deployment (5 questions)
  INSERT INTO public.capability_domains (assessment_id, name, description, order_index)
